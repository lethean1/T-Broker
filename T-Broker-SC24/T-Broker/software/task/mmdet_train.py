...
from mmdet.utils import DistOptimizerHook
try:
    import apex
except:
...
def train_detector(model,
    ...
    class MemoryThresholdHook(Hook):
        def __init__(self, pipe, dc_manager):
            self.memory_threshold_pipe = pipe
            self.dc_manager = dc_manager
            self.memory_buffer = 1
        def before_iter(self, runner):
            if self.memory_threshold_pipe.poll():
    ...
    distOptimizerHook = list(filter(lambda x: type(x) is DistOptimizerHook, runner.hooks))[0]
    memoryThresholdHook = MemoryThresholdHook(pipe, distOptimizerHook.dc_manager)
    runner.register_hook(memoryThresholdHook)
