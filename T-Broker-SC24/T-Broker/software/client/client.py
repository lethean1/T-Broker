...
def send_request(i, client, task_name, data, layers):
...
def main():
    ...
    for i in range(len(model_list)):
        client_train.append(TcpClient('localhost', int(sys.argv[2])))
        send_request(i, client_train[i], model_list[i][0], model_list[i][1], model_list[i][2])
